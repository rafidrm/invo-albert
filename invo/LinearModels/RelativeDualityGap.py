...
class RelativeDualityGap():
    ...
    def rho(self, points):
        ...
        for i in range(m):
            ...
            denomTerm = [
                np.abs(np.dot(self.A[i], point) - 1) for point in points
            ]
            denom += sum(denomTerm)
        rho = 1 - numer / denom
        return rho[0, 0]
    ...
    def _initialize_kwargs(self, kwargs):
        ...
        if 'positive_cost' in kwargs:
            assert isinstance(kwargs['positive_cost'],
                              bool), 'positive cost needs to be True or False.'
            self.positive_cost = kwargs['positive_cost']
